...
> ⚠️ **[2026-07-14] 이 문서는 대체되었다.** 0.6.43 → 0.10.2 이관이 완료되어 현재 기준 문서는 **`CUSTOMIZATIONS_0.10.2-fix1.md`**이다. 차기 버전 업그레이드는 반드시 새 문서를 기준으로 할 것. 본 문서는 0.6.43 시절의 역사적 기록으로만 보존한다. (일부 내용은 이후 뒤집혔다 — 예: Tika rmeta 방식은 0.10.2에서 채택됨, Analytics는 업스트림에 편입됨. 새 문서의 실측 내용이 우선한다.)
