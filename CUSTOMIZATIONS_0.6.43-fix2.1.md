...
6. 저장소 루트의 `migration_guide_0.6.43_fix1_to_0.7.2_fix1.md`는 **fix1 시점**의 요약이라 fix2.x에서 추가된 커스텀(기능 토글 3종, Tika 개선, 운영 스크립트, 자동 갱신 등)이 빠져 있다. **본 문서가 최신·완전판이므로 이관 작업의 기준 문서로 사용할 것.**
...
1. 파일 자체는 코드에 영향을 주지 않으므로, 저장소 루트에 `migration_guide_0.6.43_fix1_to_0.7.2_fix1.md` 파일을 그대로 복사하면 된다:
   ```bash
   git checkout <커스텀-브랜치> -- migration_guide_0.6.43_fix1_to_0.7.2_fix1.md
   ```
   또는 기존 저장소에서 파일을 통째로 복사.
2. 새 베이스 버전(예: 0.7.2 이후)으로 이관하는 경우, 파일명(버전 표기)과 내부의 대상 버전 문구를 새 버전에 맞게 갱신하는 것을 권장한다.
3. 이 문서는 다른 커스텀 코드 변경과 의존 관계가 없으므로 재적용 순서 제약 없음.

#### 사소성 판단

빌드/런타임에 전혀 영향이 없는 순수 문서 추가이므로 **기능적으로는 자명하게 사소한(non-functional) 변경**이다. 다만 커스텀 이력의 핵심 참조 문서이므로 이관 시 누락하지 말고 함께 가져가는 것이 좋다.
