...
3. **설정값은 DB(`config` 테이블)가 env보다 우선한다.** 0.10.2의 설정 구조는 부팅 시 `Config.seed_defaults(DEFAULT_CONFIG)`가 키를 DB에 심고, 이후 `Config.get()`은 항상 DB 값을 읽는다. env 변수는 **키가 DB에 없을 때의 초기값**일 뿐이다. 기존 설치본에서 env만 바꾸면 반영되지 않는다 — 실사례: `ENABLE_USER_PERSONAL_INFO=false` 설정이 무시됨. 배포 후 값이 안 먹으면 `SELECT key, value FROM config WHERE key = '<키>';`로 확인하고 `UPDATE config SET value = ...`로 직접 갱신할 것.
