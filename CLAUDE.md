...
- 코드에 추가하는 주석은 한글로 작성.
- **이 저장소는 GitHub public이다. 사내 시스템 이름·API 주소·헤더 이름/값·기관 코드 같은 내부 식별자를 코드·주석·문서·compose·커밋 메시지 어디에도 넣지 말 것.** 연동 기능은 중립적인 이름(예: `HR_SYNC_*`)으로 만들고, 실제 값은 **배포 서버의 compose 환경변수로만** 주입한다. 저장소에는 빈 기본값과 설명만 남긴다.
