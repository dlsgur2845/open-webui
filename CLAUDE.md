...
- 리눅스 amd64. **서버 시계가 실제 시간보다 약 3분 느림** (NTP 동기화 권장했으나 미적용 가정).
- 짧은 토큰 만료(`JWT_EXPIRES_IN=3m` 수준)를 사용하므로 시계 차이에 민감하다.
- **토큰/세션 판정은 항상 서버 시간이 기준.** 프론트엔드에서 브라우저 시계로 만료를 직접 판정하는 코드를 넣지 말 것. 세션 응답의 `server_timestamp`로 clockSkew를 보정하거나, 서버에 401 여부를 확인한 뒤 로그아웃할 것.
